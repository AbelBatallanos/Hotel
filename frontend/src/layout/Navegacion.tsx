    ...
    <aside className="bg-slate-900 border-r border-slate-800 h-screen w-72 flex flex-col justify-between shadow-2xl transition-all font-sans">
      ...
      <div className="p-6 border-b border-slate-800 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-white text-xl font-bold tracking-wide">
            Grand<span className="text-emerald-500">Hotel</span>
          </h1>
      ...
      >
        <ul className="space-y-2">
          ...
          {(usuarioInfo?.rol === "recepcionista" ||
            usuarioInfo?.rol === "admin" ||
            !usuarioInfo?.rol) /* Asumiendo que cliente no tiene rol específico o es otro */ && (
            <li>
              <button
                onClick={() => setShowReserva(!showReserva)}
                className={accordionBtnStyles(showReserva)}
              >
                ...
                <ChevronDownIcon
                  className={`size-4 transition-transform duration-300 ${showReserva ? "rotate-180 text-emerald-400" : ""}`}
                />
              </button>

              <ul
                className={`overflow-hidden transition-all duration-500 ease-in-out ${showReserva ? "max-h-40 mt-1 space-y-1" : "max-h-0"}`}
              >
                {usuarioInfo?.rol === "recepcionista" ||
                usuarioInfo?.rol === "admin" ? (
                  <>
          ...
          <li>
            ...
            usuarioInfo?.rol === "recepcionista" ? (
              <>
                <button
                  onClick={() => setShowHabitacion(!showHabitacion)}
                  className={accordionBtnStyles(showHabitacion)}
                >
                  <div className="flex items-center gap-3">
                  ...
                  <ChevronDownIcon
                    className={`size-4 transition-transform duration-300 ${showHabitacion ? "rotate-180 text-emerald-400" : ""}`}
                  />
                </button>
                <ul
                  className={`overflow-hidden transition-all duration-500 ease-in-out ${showHabitacion ? "max-h-40 mt-1 space-y-1" : "max-h-0"}`}
                >
                  <li>
                    <NavLink
          ...
          {usuarioInfo?.rol === "admin" && (
            <li>
              <button
                ...
                className={accordionBtnStyles(showPersonal)}
              >
                <div className="flex items-center gap-3">
                ...
                <ChevronDownIcon
                  className={`size-4 transition-transform duration-300 ${showPersonal ? "rotate-180 text-emerald-400" : ""}`}
                />
              </button>
              <ul
                className={`overflow-hidden transition-all duration-500 ease-in-out ${showPersonal ? "max-h-40 mt-1 space-y-1" : "max-h-0"}`}
              >
                <li>
      ...
      <section className="p-4 border-t border-slate-800 bg-slate-900 relative">
        ...
        <div
          className={`absolute bottom-20 left-4 right-4 bg-slate-800 border border-slate-700 rounded-xl shadow-xl transition-all duration-200 transform origin-bottom ${showDropdown ? "opacity-100 scale-100 visible" : "opacity-0 scale-95 invisible"}`}
        >
          <div className="p-2">
            <button
          ...
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className="flex flex-col truncate">
              <span className="text-white text-sm font-bold truncate">
                {usuarioInfo?.name || "Usuario"}
              </span>
              <span className="text-emerald-500 text-xs font-semibold capitalize tracking-wide">
                {usuarioInfo?.rol || "Huésped"}
              </span>
            </div>
          </div>
          <button
