...
export default function ReservacionesPage() {
  ...
  const [activeTab, setActiveTab] = useState<"Pendientes" | "Ocupados">(
    "Pendientes",
  );
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
            <div>
              ...
              <p className="text-slate-500 mt-1">
                Gestión administrativa de ingresos y ocupación en tiempo real.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {reservasGenerales[activeTab]?.map((reserva) => (
              <div
                ...
                onClick={() => {
                  if (activeTab === "Pendientes") {
                    ...
                    navigate(`/reservacion/${reserva.id}/confirmacion`, {
                      replace: true,
                    });
                  }
                }}
              ...
              >
                <div
                  className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${activeTab === "Pendientes" ? "bg-amber-50 text-amber-500" : "bg-blue-50 text-blue-500"}`}
                >
                ...
                <div className="flex-1 text-center md:text-left">
                  <div className="flex flex-col md:flex-row items-center gap-3 mb-1">
                    ...
                    <span
                      className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-md ${activeTab === "Pendientes" ? "bg-amber-100 text-amber-700" : "bg-blue-100 text-blue-700"}`}
                    >
                      {reserva.estado}
                    </span>
                  </div>
                  <div className="flex items-center justify-center md:justify-start gap-4 text-sm text-slate-500">
                    <span className="flex items-center gap-1.5 font-medium">
                      <UserIcon className="w-4 h-4" /> {reserva.cliente.name}
                    </span>
                ...
                <div className="flex items-center gap-8 px-8 md:border-l border-slate-100">
                  ...
                  <div className="text-right">
                    ...
                    <p
                      className={`font-black text-2xl ${activeTab === "Pendientes" ? "text-amber-600" : "text-blue-600"}`}
                    >
                      ${reserva.total}
                    </p>
                  </div>
