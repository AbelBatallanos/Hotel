...
export default function useHabitacion() {
  const [ListTiposHabitaciones, setListTiposHabitaciones] = useState(
    [] as TiposHabitacionT[],
  );
  const [ResponseServer, setResponseServer] = useState<ResponseServidorErrors>({
    success: false,
    messageError: "",
    msg: "",
  });
